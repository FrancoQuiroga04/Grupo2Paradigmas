...
--Para que sea del mismo tipo que el resto hago esta transaccion5, pero para que cumpla con el enunciado y las verificaciones que pide la primera parte, la estructura debería ser la otra, ver que hacer con esto
...
infinitaUsuario n (x:xs) usuario  = take n (dinero (foldr (.) id x usuario) : infinitaUsuario n xs usuario)
...
--dinero (infinitaUsuario [bloque1] pepe)

--infinitaUsuario n bloque  = foldr (.) id (take n (chainInfinita bloque))

--cantidadCreditos 57 0 1 [bloque1] pepe




--infinitaUsuario bloque = foldr (.) id (chainInfinita bloque)
...
it "¿Cómo quedaría la billetera de Pepe si le depositan 15 monedas, extrae 2, y tiene un Upgrade?" $ ((upgrade . extraccion 2 . deposito 15)(dinero pepe)) `shouldBe` 27
...
it "Aplicar la transaccion 5 a Lucho" $ transaccion5 pepe `shouldBe` 9
